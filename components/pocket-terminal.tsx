...
type View = 'trading' | 'finance' | 'profile' | 'market' | 'signals' | 'social' | 'tournaments' | 'hotkeys' | 'chat';
type Expiry = '5s' | '15s' | '30s' | '1m';
type ChartMode = 'Line' | 'Candles' | 'Bars' | 'Heikin Ashi';
type LayoutMode = '1 chart' | '2 charts' | '3 charts' | '4 charts';
...
const STORAGE_KEY = 'regnantx-pocket-terminal-v3';
...
export function PocketTerminal() {
  ...
  const [layoutMode, setLayoutMode] = useState<LayoutMode>('1 chart');
  ...
  return <main className="h-screen overflow-hidden bg-[#111528] text-white">
    <header className="flex h-12 items-center justify-between border-b border-white/10 bg-[#171c2f] px-4"><div className="flex items-center gap-3"><div className="h-7 w-7 rounded-full bg-gradient-to-r from-cyan-400 to-blue-600" /><p className="text-lg font-black">Regnant<span className="text-cyan-300">X</span></p><button className="rounded-lg border border-white/10 px-3 py-1 text-sm text-white/60">★</button></div><div className="flex items-center gap-3"><button onClick={() => setView('hotkeys')} className="rounded-xl bg-[#222940] px-3 py-2 text-xs font-bold">Hotkeys</button><div className="hidden rounded-xl bg-[#222940] px-4 py-1.5 text-right sm:block"><p className="text-[10px] text-white/35">QT Demo</p><p className="font-black text-cyan-200">{formatMoney(equity)}</p></div><button onClick={resetDemo} className="rounded-xl bg-emerald-500 px-4 py-2 text-sm font-black text-white">TOP UP</button><div className="h-9 w-9 rounded-full border border-cyan-300/40 bg-cyan-300/10" /></div></header>
    <section className="grid h-[calc(100vh-48px)] grid-cols-[72px_1fr_310px_290px] overflow-hidden"><aside className="border-r border-white/10 bg-[#171c2f] py-2"><div className="space-y-1 px-2">{menu.map((item) => <button key={item.id} onClick={() => setView(item.id)} className={cx('flex h-[48px] w-full flex-col items-center justify-center rounded-xl text-[10px] transition', view === item.id ? 'bg-cyan-400/15 text-cyan-200' : 'text-white/55 hover:bg-white/5')}><span className="text-base">{item.icon}</span><span>{item.label}</span></button>)}</div></aside>
      {view === 'trading' && <><ChartArea asset={selectedAsset} data={chartData} chartMode={chartMode} setChartMode={setChartMode} layoutMode={layoutMode} setLayoutMode={setLayoutMode} /><TradePanel asset={selectedAsset} expiry={expiry} setExpiry={setExpiry} amount={amount} setAmount={setAmount} openTrade={openTrade} /><RightPanel positions={livePositions} closeTrade={closeTrade} assets={filtered} selected={selected} setSelected={setSelected} search={search} setSearch={setSearch} /></>}
...
function ChartArea({ asset, data, chartMode, setChartMode, layoutMode, setLayoutMode }: { asset: LiveAsset; data: Array<{ t: number; price: number }>; chartMode: ChartMode; setChartMode: (m: ChartMode)=>void; layoutMode: LayoutMode; setLayoutMode: (m: LayoutMode)=>void }) { return <section className="relative overflow-hidden bg-[#12182b]"><div className="flex h-12 items-center gap-2 border-b border-white/10 bg-[#171c2f] px-4"><button className="rounded-lg bg-[#29324d] px-3 py-2 font-bold">{asset.symbol}/USDT ▾</button>{['📈','⚙','✎','⋯','▦'].map((x) => <button key={x} className="h-9 w-9 rounded-lg bg-[#29324d] text-white/80">{x}</button>)}</div><div className="pointer-events-none absolute inset-x-0 top-12 z-10 flex items-start justify-between px-4 py-3"><div><p className="text-xs text-white/40">{asset.name}</p><h1 className="text-3xl font-black">{asset.symbol}/USDT</h1></div><div className="text-right"><p className="text-2xl font-black">{priceFormat(asset.price)}</p><p className={asset.liveChange >= 0 ? 'text-emerald-300' : 'text-rose-300'}>{asset.liveChange >= 0 ? '+' : ''}{asset.liveChange.toFixed(2)}%</p></div></div><ToolStrip /><div className={cx('h-[calc(100%-48px)] bg-[linear-gradient(rgba(255,255,255,.045)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.045)_1px,transparent_1px)] bg-[size:80px_80px] px-4 pt-16', layoutMode !== '1 chart' && 'grid gap-2', layoutMode === '2 charts' && 'grid-cols-2', layoutMode === '3 charts' && 'grid-cols-2', layoutMode === '4 charts' && 'grid-cols-2')}><TradingChart data={data} positive={asset.liveChange >= 0} mode={chartMode} />{layoutMode !== '1 chart' && <TradingChart data={data.slice().reverse()} positive={asset.liveChange < 0} mode={chartMode} />}{(layoutMode === '3 charts' || layoutMode === '4 charts') && <TradingChart data={data.map((x)=>({...x, price:x.price*1.004}))} positive mode={chartMode} />}{layoutMode === '4 charts' && <TradingChart data={data.map((x)=>({...x, price:x.price*0.997}))} positive={false} mode={chartMode} />}</div><ChartSettings chartMode={chartMode} setChartMode={setChartMode} layoutMode={layoutMode} setLayoutMode={setLayoutMode} /><div className="absolute bottom-4 left-4 rounded-xl bg-[#202841] px-4 py-2 text-sm font-bold">M6⌃</div></section>; }
...
function ChartSettings({ chartMode, setChartMode, layoutMode, setLayoutMode }: { chartMode: ChartMode; setChartMode: (m: ChartMode)=>void; layoutMode: LayoutMode; setLayoutMode: (m: LayoutMode)=>void }) { const modes: ChartMode[] = ['Line','Candles','Bars','Heikin Ashi']; const layouts: LayoutMode[] = ['1 chart','2 charts','3 charts','4 charts']; return <div className="absolute bottom-4 right-4 z-20 rounded-2xl border border-white/10 bg-[#202841]/95 p-3"><p className="mb-2 text-xs text-white/40">Chart types</p><div className="grid grid-cols-4 gap-2">{modes.map((m)=><button key={m} onClick={()=>setChartMode(m)} className={cx('rounded-xl px-3 py-2 text-xs font-bold', chartMode===m?'bg-cyan-400 text-black':'bg-white/5 text-white/60')}>{m}</button>)}</div><p className="mb-2 mt-3 text-xs text-white/40">Layout</p><div className="grid grid-cols-4 gap-2">{layouts.map((m)=><button key={m} onClick={()=>setLayoutMode(m)} className={cx('rounded-xl px-2 py-2 text-[10px] font-bold', layoutMode===m?'bg-cyan-400 text-black':'bg-white/5 text-white/60')}>{m}</button>)}</div></div>; }
...
function UtilityView({ view, balance, assets, selected, setSelected, search, setSearch }: { view: View; balance: number; assets: LiveAsset[]; selected: string; setSelected: (s:string)=>void; search: string; setSearch: (s:string)=>void }) {
  ...
  if (view === 'social') return <SocialTrading />;
  if (view === 'hotkeys') return <Hotkeys />;
  ...
  if (view === 'tournaments') return <Tournaments />;
...
function SocialTrading() { const users = ['Trading','Aizat','BanDa','KingKilo','DmacGFL','user120738702','user131507415']; return <div className="grid max-w-2xl gap-2">{users.map((u,i)=><div key={u} className="grid grid-cols-[48px_1fr_auto] items-center gap-3 rounded-xl bg-[#202841] p-3"><div className="h-11 w-11 rounded-full bg-cyan-400/20"/><div><b>{u}</b><p className="text-xs text-white/40">Number of trades: {46+i*73}</p></div><div className="text-right text-emerald-300"><b>+${(8711-i*720).toLocaleString('en-US')}</b><p className="text-xs text-white/40">{100-i*7}% profitable</p></div></div>)}</div>; }
function Hotkeys() { const keys = [['Shift + W','Buy'],['Shift + S','Sell'],['Shift + A','Decrease amount'],['Shift + D','Increase amount'],['Shift + Tab','Next favorite asset']]; return <div className="max-w-xl rounded-2xl bg-[#202841] p-5"><h2 className="mb-4 text-2xl font-black">Hotkeys [Inactive]</h2>{keys.map(([k,v])=><div key={k} className="flex justify-between border-b border-white/10 py-3"><span className="rounded-lg bg-[#111528] px-3 py-2 text-cyan-200">{k}</span><span>{v}</span></div>)}<button className="mt-5 w-full rounded-xl border border-cyan-400/40 py-3">Activate hotkeys</button></div>; }
function Tournaments() { const rows = ['Day off · Prize $250 · Free','Hour play · Prize $100 · Fee $1','Rumble · Prize $2,500 · Fee $25','Night sprint · Prize $500 · Fee $3']; return <div className="grid max-w-3xl gap-4">{rows.map((x)=><div key={x} className="rounded-2xl bg-[#202841] p-5"><b>{x}</b><button className="float-right rounded-xl bg-emerald-600 px-8 py-2 font-bold">Join</button><p className="mt-3 text-sm text-white/40">Ends in: 1h 38m 3s</p></div>)}</div>; }
