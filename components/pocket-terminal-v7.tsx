...
const STORE = 'regnantx-terminal-v7';
...
function TradingViewChart({ asset, chartType }: { asset: Asset; chartType: ChartType }) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(wrapRef.current, {
      ...
      layout: { background: { type: ColorType.Solid, color: '#12182b' }, textColor: 'rgba(255,255,255,.55)' },
      grid: { vertLines: { color: 'rgba(255,255,255,.06)' }, horzLines: { color: 'rgba(255,255,255,.06)' } },
      rightPriceScale: { borderColor: 'rgba(255,255,255,.1)' },
      timeScale: { borderColor: 'rgba(255,255,255,.1)', timeVisible: true, secondsVisible: true },
      crosshair: { mode: 1 },
    });
...
export function PocketTerminalV7() {
  ...
  function openTrade(side: Side) {
    ...
    setNotice(`${side === 'LONG' ? 'BUY' : 'SELL'} opened · ${asset.symbol}`);
  }
  ...
  return <main className="h-screen overflow-hidden bg-[#101524] text-white">
    ...
    <section className="grid h-[calc(100vh-48px)] grid-cols-[72px_1fr_310px_300px] overflow-hidden">
      <aside className="border-r border-white/10 bg-[#171d30] p-2">{['Trade','Finance','Market','Signals','Social','History'].map((x, i) => <button key={x} className={cx('mb-1 flex h-[54px] w-full flex-col items-center justify-center rounded-xl text-[9px]', i === 0 ? 'bg-cyan-400/15 text-cyan-200' : 'text-white/55')}>↗<span>{x}</span></button>)}</aside>
      <section className="relative bg-[#12182b]"><div className="flex h-12 items-center gap-2 border-b border-white/10 bg-[#171d30] px-4"><b className="rounded-lg bg-[#29324d] px-3 py-2">{asset.symbol}/USDT</b>{(['Candles','Line','Bars'] as ChartType[]).map((t) => <button key={t} onClick={() => setChartType(t)} className={cx('rounded-lg px-3 py-2 text-xs font-bold', chartType === t ? 'bg-cyan-400 text-black' : 'bg-[#29324d]')}>{t}</button>)}<button className="rounded-lg bg-[#29324d] px-3 py-2 text-xs font-bold">RSI</button><button className="rounded-lg bg-[#29324d] px-3 py-2 text-xs font-bold">MACD</button></div><div className="absolute left-4 top-16 z-10"><p className="text-xs text-white/40">{asset.name} {asset.live && <span className="text-cyan-200">· Binance live</span>}</p><h1 className="text-3xl font-black">{fmt(asset.price)}</h1><p className={asset.liveChange >= 0 ? 'text-emerald-300' : 'text-rose-300'}>{asset.liveChange >= 0 ? '+' : ''}{asset.liveChange.toFixed(2)}%</p></div><div className="h-[calc(100%-48px)] p-4 pt-20"><TradingViewChart asset={asset} chartType={chartType} /></div></section>
      <aside className="border-l border-white/10 bg-[#22283c] p-4"><p className="text-xs text-white/40">Payout</p><div className="mb-4 rounded-xl border border-emerald-400/20 bg-black/20 p-3 text-center"><p className="text-emerald-300">+{asset.payout}%</p><p className="text-xs text-white/40">+{money(amount * asset.payout / 100)}</p></div><p className="text-xs text-white/45">Time</p><div className="mt-2 grid grid-cols-4 gap-2">{(['5s','15s','30s','1m'] as Expiry[]).map((e) => <button key={e} onClick={() => setExpiry(e)} className={cx('rounded-lg py-2 text-xs font-bold', expiry === e ? 'bg-cyan-400 text-black' : 'bg-[#171c2f] text-white/60')}>{e}</button>)}</div><p className="mt-4 text-xs text-white/45">Amount</p><input value={amount} onChange={(e) => setAmount(Number(e.target.value))} type="number" className="mt-2 w-full rounded-xl border border-white/10 bg-[#171c2f] px-4 py-3 text-xl font-black outline-none" /><button onClick={() => openTrade('LONG')} className="mt-4 w-full rounded-xl bg-[#31b545] py-4 text-lg font-black">BUY</button><button onClick={() => openTrade('SHORT')} className="mt-3 w-full rounded-xl bg-[#ff3b30] py-4 text-lg font-black">SELL</button></aside>
      <aside className="border-l border-white/10 bg-[#1b2135]"><div className="border-b border-white/10 p-3 text-center font-bold">Trades</div><div className="max-h-[260px] overflow-auto p-3">{livePositions.length === 0 ? <p className="py-6 text-center text-sm text-white/45">No trades</p> : livePositions.map((p) => { const left = Math.max(0, Math.ceil((p.closesAt - Date.now()) / 1000)); return <div key={p.id} className="mb-2 rounded-xl bg-[#252c43] p-3"><div className="flex justify-between"><b>{p.symbol}</b><span className={p.pnl >= 0 ? 'text-emerald-300' : 'text-rose-300'}>{money(p.pnl)}</span></div><p className="text-xs text-white/45">{p.side} · closes in {left}s · {money(p.amount)}</p><button onClick={() => settleTrade(p)} className="mt-2 rounded-lg bg-white/10 px-3 py-1 text-xs">Close</button></div>; })}</div><div className="border-y border-white/10 p-3"><b className="text-sm">Closed History</b>{closed.slice(0, 4).map((p) => <p key={p.id} className="mt-2 text-xs text-white/55">{p.symbol} · {p.result} · <span className={p.pnl >= 0 ? 'text-emerald-300' : 'text-rose-300'}>{money(p.pnl)}</span></p>)}</div><div className="border-b border-white/10 p-3"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search assets" className="w-full rounded-lg bg-[#111528] px-3 py-2 text-sm outline-none" /></div><div className="max-h-[calc(100vh-430px)] overflow-auto p-3">{filtered.slice(0, 12).map((a) => <button key={a.symbol} onClick={() => setSelected(a.symbol)} className={cx('mb-2 grid w-full grid-cols-[1fr_auto] rounded-xl p-3 text-left', selected === a.symbol ? 'bg-cyan-400/15' : 'bg-[#252c43] hover:bg-[#303851]')}><span><b>{a.symbol}</b><p className="text-xs text-white/40">{a.name}</p></span><span className="text-sm text-cyan-200">{a.live ? fmt(a.price) : `+${a.payout}%`}</span></button>)}</div></aside>
    </section>
