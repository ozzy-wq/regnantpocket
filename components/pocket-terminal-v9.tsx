...
const STORE = 'regnantx-terminal-v9';
...
function clampPrice(price: number, reference: number, limit = 0.03) {
  if (!reference || reference <= 0) return price;
  const min = reference * (1 - limit);
  const max = reference * (1 + limit);
...
function seedCandles(currentPrice: number): CandlestickData[] {
  ...
  return Array.from({ length: 90 }, (_, i) => {
    ...
    const drift = Math.sin(i / 8) * volatility * 0.9;
    const open = previousClose;
    const rawClose = currentPrice + drift + (Math.random() - 0.5) * volatility;
...
function SafeChart({ asset, chartType }: { asset: Asset; chartType: ChartType }) {
  ...
  const symbolRef = useRef(asset.symbol);
  ...
  useEffect(() => {
    ...
    if (!chart) return;
    symbolRef.current = asset.symbol;
  ...
  useEffect(() => {
    ...
    const high = Math.max(open, close) + Math.random() * volatility * 0.35;
    const low = Math.min(open, close) - Math.random() * volatility * 0.35;
    const next: CandlestickData = { time, open, high, low, close };
...
export function PocketTerminalV9() {
