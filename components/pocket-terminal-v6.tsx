...
type Panel = 'terminal' | 'finance' | 'market' | 'signals' | 'social' | 'news' | 'calendar' | 'history';
type Expiry = '5s' | '15s' | '30s' | '1m';
type ChartType = 'Line' | 'Candles' | 'Bars';
type Position = { id: string; symbol: string; side: Side; amount: number; entry: number; current: number; pnl: number; expiry: Expiry; openedAt: number; closesAt: number; status: 'open' | 'pending' | 'closed'; result?: 'win' | 'loss' | 'draw' };
...
const STORE = 'regnantx-terminal-v6';
...
export function PocketTerminalV6() {
  ...
  const assets: Asset[] = useMemo(() => baseAssets.map((a, i) => {
    ...
    return { ...a, price: live || simulatedPrice, liveChange: a.change + drift * 100, payout: payout(a.score), live: Boolean(live) };
  }), [tick, livePrices]);
  ...
  useEffect(() => {
    ...
    expired.forEach((p) => settleTrade(p));
  }, [tick, livePositions]);
  ...
  useEffect(() => {
    if (!auto) return;
    const t = setInterval(() => openTrade(asset.liveChange >= 0 ? 'LONG' : 'SHORT', 50, 'pending'), 9000);
    return () => clearInterval(t);
  ...
  function openTrade(side: Side, customAmount = amount, status: Position['status'] = 'open') {
    ...
    const now = Date.now();
    const next: Position = { id: `${now}-${Math.random()}`, symbol: asset.symbol, side, amount: customAmount, entry: asset.price, current: asset.price, pnl: 0, expiry, openedAt: now, closesAt: now + expiryMs(expiry), status };
  ...
  function settleTrade(item: Position) {
    ...
    setNotice(`${item.symbol} ${result.toUpperCase()} · ${money(item.pnl)}`);
  }
  function closeTrade(id: string) { const item = livePositions.find((p) => p.id === id); if (item) settleTrade(item); }
  function activate(id: string) { const now = Date.now(); setPositions((v) => v.map((p) => p.id === id ? { ...p, status: 'open', openedAt: now, closesAt: now + expiryMs(p.expiry) } : p)); }
  ...
  return <main className="h-screen overflow-hidden bg-[#101524] text-white">
    <header className="flex h-12 items-center justify-between border-b border-white/10 bg-[#171d30] px-4">
      ...
      <div className="flex items-center gap-2"><Pill label="LIVE" value={liveStatus === 'live' ? 'ON' : liveStatus.toUpperCase()} active={liveStatus === 'live'} /><Pill label="AI" value={`${asset.payout}%`} active /><button onClick={() => setAuto((v) => !v)} className={cx('rounded-xl px-3 py-2 text-xs font-black', auto ? 'bg-cyan-400 text-black' : 'bg-[#252c43]')}>AI AUTO {auto ? 'ON' : 'OFF'}</button><div className="rounded-xl bg-[#252c43] px-4 py-1 text-right"><p className="text-[10px] text-white/35">QT Demo</p><b className="text-cyan-200">{money(equity)}</b></div><button onClick={reset} className="rounded-xl bg-emerald-500 px-4 py-2 text-sm font-black">RESET</button></div>
    </header>
...
function Chart({ asset, candles, chartType, setChartType }: { asset: Asset; candles: Candle[]; chartType: ChartType; setChartType: (v: ChartType) => void }) { return <section className="relative bg-[#12182b]"><div className="flex h-12 items-center gap-2 border-b border-white/10 bg-[#171d30] px-4"><b className="rounded-lg bg-[#29324d] px-3 py-2">{asset.symbol}/USDT</b>{(['Line','Candles','Bars'] as ChartType[]).map((t) => <button key={t} onClick={() => setChartType(t)} className={cx('rounded-lg px-3 py-2 text-xs font-bold', chartType === t ? 'bg-cyan-400 text-black' : 'bg-[#29324d]')}>{t}</button>)}<button className="rounded-lg bg-[#29324d] px-3 py-2 text-xs font-bold">RSI</button><button className="rounded-lg bg-[#29324d] px-3 py-2 text-xs font-bold">MACD</button><button className="rounded-lg bg-[#29324d] px-3 py-2 text-xs font-bold">Fibo</button></div><div className="absolute left-4 top-16 z-10"><p className="text-xs text-white/40">{asset.name} {asset.live && <span className="text-cyan-200">· Binance live</span>}</p><h1 className="text-3xl font-black">{fmt(asset.price)}</h1><p className={asset.liveChange >= 0 ? 'text-emerald-300' : 'text-rose-300'}>{asset.liveChange >= 0 ? '+' : ''}{asset.liveChange.toFixed(2)}%</p></div><div className="absolute right-4 top-16 z-10 w-60 rounded-2xl border border-cyan-400/20 bg-[#202841]/90 p-3"><b className="text-cyan-200">AI BOT</b><p className={asset.liveChange >= 0 ? 'mt-2 text-emerald-300' : 'mt-2 text-rose-300'}>{asset.liveChange >= 0 ? 'BUY pressure detected' : 'SELL pressure detected'}</p><div className="mt-3 h-2 rounded-full bg-black/30"><div className="h-2 rounded-full bg-cyan-400" style={{ width: `${asset.payout}%` }} /></div><p className="mt-2 text-xs text-white/40">Confidence {asset.payout}%</p></div><div className="h-[calc(100%-48px)] bg-[linear-gradient(rgba(255,255,255,.045)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.045)_1px,transparent_1px)] bg-[size:80px_80px] p-4 pt-20"><MiniChart candles={candles} chartType={chartType} positive={asset.liveChange >= 0} /></div></section>; }
...
function PanelView({ panel, balance, assets, selected, setSelected, search, setSearch, closed, feed }: { panel: Panel; balance: number; assets: Asset[]; selected: string; setSelected: (s: string) => void; search: string; setSearch: (s: string) => void; closed: Position[]; feed: Feed[] }) { if (panel === 'market') return <Market assets={assets} selected={selected} setSelected={setSelected} search={search} setSearch={setSearch} />; if (panel === 'finance' || panel === 'history') return <Finance balance={balance} closed={closed} />; if (panel === 'signals') return <Signals assets={assets} setSelected={setSelected} />; if (panel === 'social') return <Social />; if (panel === 'news') return <Simple items={['BTC ETF inflows remain strong', 'ETH volatility rises before US session', 'SOL breaks short term resistance', 'Market waits for CPI data']} />; if (panel === 'calendar') return <Simple items={['15:30 · US CPI · High impact', '16:45 · Manufacturing PMI · Medium', '18:00 · Fed speaker · High']} />; return <Market assets={assets} selected={selected} setSelected={setSelected} search={search} setSearch={setSearch} />; }
...
function Finance({ balance, closed }: { balance: number; closed: Position[] }) { return <div><div className="mb-6 rounded-2xl bg-[#202841] p-5"><p className="text-white/45">Demo balance</p><p className="text-4xl font-black text-cyan-200">{money(balance)}</p></div><h2 className="mt-8 text-xl font-black">Wallet / Trade History</h2>{closed.length === 0 ? <p className="mt-3 text-white/45">No wallet history yet.</p> : closed.map((x) => <div key={x.id} className="mt-2 flex max-w-3xl justify-between rounded-xl bg-[#202841] p-3"><span>{x.symbol} · {x.side} · {x.result}</span><span className={x.pnl >= 0 ? 'text-emerald-300' : 'text-rose-300'}>{money(x.pnl)}</span></div>)}</div>; }
