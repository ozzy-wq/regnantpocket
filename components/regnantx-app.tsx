...
export function RegnantXApp() {
  ...
  const [activity, setActivity] = useState<Activity[]>([
    { id: 'a1', text: 'AI signal engine initialized', time: 'System', tone: 'gold' },
    { id: 'a2', text: 'Demo risk guard enabled', time: 'System', tone: 'green' }
  ]);
  ...
  function pushActivity(text: string, tone: Activity['tone'] = 'neutral') { setActivity((v) => [{ id: `${Date.now()}-${Math.random()}`, text, time: timeStamp(), tone }, ...v].slice(0, 8)); }
  function openPosition(side: Side, customAmount = amount, source = 'Manual') {
    ...
    setBalance((v) => v - customAmount); setPositions((v) => [next, ...v]); setToast(`${side} opened on ${selectedAsset.symbol}`); pushActivity(`${side} opened on ${selectedAsset.symbol} for ${formatMoney(customAmount)}`, side === 'LONG' ? 'green' : 'red');
  }
  function closePosition(id: string) { const item = livePositions.find((p) => p.id === id); if (!item) return; setBalance((v) => v + item.amount + item.pnl); setPositions((v) => v.filter((p) => p.id !== id)); setHistory((v) => [{ ...item, source: `${item.source} · Closed` }, ...v]); setToast(`${item.symbol} closed · ${formatMoney(item.pnl)}`); pushActivity(`${item.symbol} closed with PnL ${formatMoney(item.pnl)}`, item.pnl >= 0 ? 'green' : 'red'); }
  function resetDemo() { setBalance(STARTING_BALANCE); setPositions([]); setHistory([]); setRisk('Balanced'); localStorage.removeItem(STORAGE_KEY); setToast('Demo reset complete.'); pushActivity('Demo wallet reset', 'gold'); }
  function addAlert() { if (!alertText.trim()) return; setAlerts((v) => [alertText.trim(), ...v]); setAlertText(''); setToast('Alert added.'); pushActivity(`Alert created: ${alertText.trim()}`, 'gold'); }
  function copyReport() { navigator.clipboard?.writeText(`REGNANT X DEMO REPORT\nBalance: ${formatMoney(balance)}\nOpen PnL: ${formatMoney(openPnl)}\nEquity: ${formatMoney(equity)}\nOpen positions: ${positions.length}\nClosed trades: ${history.length}\nRisk mode: ${risk}\nFear & Greed: ${fg}`); setToast('Report copied.'); }

  return <main className="min-h-screen bg-bg pb-24 text-white lg:pb-0"><div className="pointer-events-none fixed inset-0 -z-10 bg-[radial-gradient(circle_at_top_left,rgba(245,158,11,.18),transparent_30%),radial-gradient(circle_at_top_right,rgba(37,99,235,.16),transparent_35%)]" />
...
function ActivityFeed({ items }: { items: Activity[] }) { return <Card><h2 className="text-2xl font-black">Recent Activity</h2><div className="mt-4 space-y-2">{items.map((item) => <div key={item.id} className="rounded-2xl border border-white/10 bg-black/20 p-3"><div className="flex items-center justify-between gap-3"><p className="text-sm font-bold">{item.text}</p><Pill tone={item.tone === 'neutral' ? 'neutral' : item.tone}>{item.time}</Pill></div></div>)}</div></Card>; }
